...
public class ExercisesTile extends VBox {
	...
	public void showOverview() {
		/* No cutting of, of description, should be not soooooo long
		String exerciseName = (exercise.getName().length() > 50) ? exercise.getName().substring(0, 49).trim() + "..." : exercise.getName().trim();
		header.setText(exerciseName);
		String exerciseDescription = (exercise.getDescription().length() > 100) ? exercise.getDescription().substring(0, 99).trim() + "..." : exercise.getDescription().trim();
